...
export default function AdminDashboard() {
  ...
  const handleVerifyKYC = async (userId: string, approved: boolean) => {
    try {
      setProcessingTransactions(prev => ({ ...prev, [userId]: true }));
      await apiClient.verifyKYC(userId, approved, approved ? 'Approved by admin' : 'Rejected by admin');
      setSuccessMessage(`KYC ${approved ? 'approved' : 'rejected'} successfully!`);
  ...
  const statsCards = [
    ...
    { icon: Wallet, label: 'Total Invested', value: stats.totalInvested || '$0', change: '+$2.1M this week' },
    { icon: TrendingUp, label: 'Total Earnings Generated', value: stats.totalEarnings || '$0', change: '+$450K today' },
    { icon: CheckCircle, label: 'Verified Users', value: stats.verifiedUsers?.toLocaleString() || '0', change: '85.6% of total' },
  ];
              ...
              <div className='rounded-lg bg-card border border-border'>
                ...
                {allWithdrawals.length > 0 ? (
                  <div className='divide-y divide-border'>
                    {allWithdrawals.filter((withdrawal: any) => withdrawal.status === withdrawalFilter).map((withdrawal: any) => (
                      <div key={withdrawal._id} className='p-4 md:p-6 hover:bg-muted/50 transition-colors'>
                        <div className='grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-3 md:mb-4'>
                          ...
                          <div className='col-span-2 md:col-span-1'>
                            <p className='text-[10px] md:text-xs text-muted-foreground mb-1'>Wallet</p>
                            <p className='font-mono text-[10px] md:text-xs text-foreground break-all'>{withdrawal.walletAddress || 'N/A'}</p>
                          </div>
                        </div>
                        ...
                        <div className='flex flex-wrap gap-2'>
                          ...
                          >
                            {processingTransactions[withdrawal._id] ? '...' : 'Processing'}
                          </button>
                          <button
                            ...
                            className='flex-1 md:flex-none px-3 md:px-4 py-2 rounded bg-green-500/20 text-green-600 hover:bg-green-500 hover:text-white disabled:opacity-50 font-semibold transition-all text-xs md:text-sm'
                          >
                            {processingTransactions[withdrawal._id] ? '...' : 'Paid Out'}
                          </button>
                        </div>
                      </div>
                    ))}
                      ...
                      <div key={user._id} className='p-4 md:p-6 hover:bg-muted/50 transition-colors'>
                        ...
                        <div className='grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 text-xs'>
                          <div>
                            ...
                            <p className='font-semibold text-foreground'>${user.balance?.toFixed(2) || '0.00'}</p>
                          </div>
                          <div>
                            <p className='text-muted-foreground mb-1'>KYC Status</p>
                            <p className={`font-semibold ${user.kycStatus === 'approved' ? 'text-green-600' : user.kycStatus === 'rejected' ? 'text-red-600' : 'text-yellow-600'}`}>
                              {user.kycStatus?.toUpperCase() || 'PENDING'}
                            </p>
                          </div>
                          <div>
                            <p className='text-muted-foreground mb-1'>Investments</p>
                            <p className='font-semibold text-foreground'>{user.investmentsCount || 0}</p>
                          </div>
                          <div>
                            <p className='text-muted-foreground mb-1'>Joined</p>
                            <p className='font-semibold text-foreground text-[10px]'>{new Date(user.createdAt).toLocaleDateString()}</p>
                          </div>
                        </div>
                        <div className='flex flex-wrap gap-2'>
                          <button
                      ...
                      <div key={user._id} className='p-4 md:p-6 hover:bg-muted/50 transition-colors'>
                        ...
                        <div className='grid grid-cols-2 gap-3 md:gap-4 mb-3 md:mb-4'>
                          <div>
                            ...
                            <p className='font-semibold text-foreground text-sm md:text-base'>{user.phone || 'N/A'}</p>
                          </div>
                          <div>
                            <p className='text-[10px] md:text-xs text-muted-foreground mb-1'>Applied</p>
                            <p className='font-semibold text-foreground text-sm md:text-base'>{user.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'N/A'}</p>
                          </div>
                        </div>
                        <div className='flex gap-2'>
                          <button
                            ...
                            className='flex-1 md:flex-none px-3 md:px-4 py-2 rounded bg-green-500/20 text-green-600 hover:bg-green-500 hover:text-white disabled:opacity-50 font-semibold transition-all text-xs md:text-sm'
                          >
                            {processingTransactions[user._id] ? '...' : 'Approve'}
                          </button>
                          <button
                            onClick={() => handleVerifyKYC(user._id, false)}
                            disabled={processingTransactions[user._id]}
                            className='flex-1 md:flex-none px-3 md:px-4 py-2 rounded bg-red-500/20 text-red-600 hover:bg-red-500 hover:text-white disabled:opacity-50 font-semibold transition-all text-xs md:text-sm'
                          >
                            Reject
