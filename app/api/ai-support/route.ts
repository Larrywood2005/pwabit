...
const generateAIResponse = (userMessage: string, conversationHistory: any[] = []): string => {
  ...
  // CONTEXT-AWARE LOGIC: Check if user is asking follow-up questions
  const hasFollowUp = recentContext.length > 0;
  
  // Extract topics from conversation history to make responses more relevant
  const mentionedTopics = {
    investment: recentContext.includes('invest') || lowerMessage.includes('invest'),
    withdrawal: recentContext.includes('withdraw') || lowerMessage.includes('withdraw'),
    trading: recentContext.includes('trade') || lowerMessage.includes('trade'),
    kyc: recentContext.includes('kyc') || lowerMessage.includes('kyc'),
    security: recentContext.includes('secure') || recentContext.includes('safe') || lowerMessage.includes('secure'),
    support: recentContext.includes('help') || lowerMessage.includes('help')
  };
  ...
  if (mentionedTopics.investment || lowerMessage.includes('package') || lowerMessage.includes('plans')) {
    const response = 'Powabitz offers three investment packages:\n• BRONZE: $100-$500 (5% daily return)\n• SILVER: $500-$2,000 (8% daily return)\n• GOLD: $2,000+ (12% daily return)\n\nYou can invest in USD or NGN (₦1,420 per $1). Go to Dashboard > Invest to choose your package today!';
    
    // Add follow-up if in conversation history
    if (hasFollowUp && mentionedTopics.investment) {
      return response + '\n\nBased on our conversation, are you interested in a specific package or would you like to know more about minimum investments?';
    }
    return response;
  }
  ...
  // Profits & Earnings
  if (mentionedTopics.investment || lowerMessage.includes('return') || lowerMessage.includes('profit') || lowerMessage.includes('earning') || lowerMessage.includes('gains')) {
  ...
  // Withdrawals
  if (mentionedTopics.withdrawal || lowerMessage.includes('withdraw') || lowerMessage.includes('cash out') || lowerMessage.includes('payout')) {
  ...
  // KYC Verification
  if (mentionedTopics.kyc || lowerMessage.includes('kyc') || lowerMessage.includes('verification') || lowerMessage.includes('verify')) {
  ...
  if (mentionedTopics.security || lowerMessage.includes('safe') || lowerMessage.includes('security') || lowerMessage.includes('protected')) {
  ...
  if (mentionedTopics.support || lowerMessage.includes('help') || lowerMessage.includes('support') || lowerMessage.includes('issue') || lowerMessage.includes('problem')) {
  ...
  if (hasFollowUp) {
    return 'I understand you\'re asking more about our platform. Here are some popular topics:\n• Investment Packages & Returns\n• How to Make Deposits\n• Withdrawals & Payments\n• Puzzle Games & Bonuses\n• PowaUp Trading\n• Account Security\n\nWhich topic interests you most? I\'ll provide more detailed information based on your needs.';
  }
