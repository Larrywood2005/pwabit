  ...
  return (
    ...
    >
      ...
      >
        ...
        {candles.filter((item, i) => isMobile ? i % 3 === 0 : i % 2 === 0).map((item, idx) => {
        ...
        {/* Candlesticks */}
        {candles && candles.length > 0 && candles.map((candleData, candleIndex) => {
          const candleX = leftPadding + candleIndex * candleWidth + candleSpacing / 2;
          const yHigh = priceToY(candleData.high);
          const yLow = priceToY(candleData.low);
          const yOpen = priceToY(candleData.open);
          const yClose = priceToY(candleData.close);
          ...
          return (
            <g key={`candle-${candleIndex}`}>
              {/* Wick */}
              <line
                x1={candleX + actualCandleWidth / 2}
                y1={yHigh}
                x2={candleX + actualCandleWidth / 2}
                y2={yLow}
              ...
              <rect
                ...
                opacity={isTrading && candleIndex === candles.length - 1 ? 0.8 : 1}
              />
            </g>
          );
        })}
