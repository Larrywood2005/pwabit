...
export function AICustomerSupport() {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className='fixed bottom-6 right-6 z-40 p-4 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-lg hover:shadow-xl transition-shadow hover:scale-110'
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 w-96 bg-card border border-border rounded-lg shadow-2xl transition-all ${
      isMinimized ? 'h-12' : 'h-[600px]'
    }`}>
      {/* Header */}
      <div className='flex items-center justify-between p-4 border-b border-border bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-t-lg'>
        <div>
          <h3 className='font-bold text-sm'>Powabitz Support</h3>
          <p className='text-xs opacity-90'>Powered by Powabitz</p>
        </div>
          ...
          <form onSubmit={handleSendMessage} className='p-4 border-t border-border'>
            <div className='flex gap-2'>
              <input
                ...
                disabled={isLoading}
                className='flex-1 px-3 py-2 rounded-lg bg-background border border-border text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-purple-600 disabled:opacity-50'
              />
              <button
                type='submit'
