...
router.post('/:id/trade', authenticate, async (req, res) => {
  try {
    ...
    console.error('[v0] Trade error:', {
      message: error.message,
      stack: error.stack,
      userId: req.user?.userId,
      investmentId: req.params.id
    });
    res.status(500).json({ message: 'Failed to place trade. Please try again.' });
  }
});
...
router.post('/:id/withdraw', authenticate, async (req, res) => {
  try {
    ...
    // CRITICAL: Mark fundsDebited=true because we are deducting from user's balance NOW
    const transaction = new Transaction({
      userId: req.user.userId,
      type: 'withdrawal',
      ...
      fundsDebited: true, // CRITICAL: Funds are being deducted at withdrawal request
    ...
    // Deduct from user balance - FUNDS ARE PERMANENTLY REMOVED
    // They will only be returned if admin REJECTS the withdrawal
    user.currentBalance -= totalWithdrawAmount;
    // Note: We don't add to totalWithdrawn until admin CONFIRMS the withdrawal
    await user.save();
    
    console.log('[v0] Withdrawal request created - Funds debited from user:', {
      userId: user._id,
      amount: totalWithdrawAmount,
...
router.get('/balance/info', authenticate, async (req, res) => {
  ...
  try {
    ...
    // USE SINGLE SOURCE OF TRUTH: balanceService.calculateAvailableBalance
    // This ensures withdrawal, purchase, and wallet endpoints all use the same logic
    const balanceInfo = await balanceService.calculateAvailableBalance(req.user.userId);

    console.log('[balance/info] Using balanceService for SINGLE SOURCE OF TRUTH:', {
      userId: req.user.userId,
      ...
      message: 'All balance values calculated by balanceService'
    });

    res.json({
      // Main balances - SINGLE SOURCE OF TRUTH from balanceService
      totalBalance: Math.max(0, balanceInfo.totalBalance || 0),
      availableBalance: Math.max(0, balanceInfo.availableBalance || 0),
      lockedInTrades: Math.max(0, balanceInfo.lockedInTrades || 0),
      pendingWithdrawal: Math.max(0, balanceInfo.pendingWithdrawal || 0),
      
      // Earnings breakdown
      earnings: Math.max(0, balanceInfo.investmentReturns || 0),
      totalEarnings: Math.max(0, balanceInfo.totalEarnings || 0),
      investmentReturns: Math.max(0, balanceInfo.investmentReturns || 0),
      puzzleGameBonuses: Math.max(0, balanceInfo.puzzleGameBonuses || 0),
      tradingBonuses: Math.max(0, balanceInfo.tradingBonuses || 0),
      referralEarnings: Math.max(0, balanceInfo.referralEarnings || 0),
      activityEarnings: Math.max(0, balanceInfo.activityEarnings || 0),
      
      // Investment info
      totalInvested: Math.max(0, balanceInfo.totalInvested || 0),
      investments: Math.max(0, balanceInfo.investmentCount || 0) || 0,
      
      // User account info
      powaUpBalance: Math.max(0, balanceInfo.powaUpBalance || 0),
      totalDeposited: Math.max(0, user.totalDeposited || 0),
      totalWithdrawn: Math.max(0, user.totalWithdrawn || 0),
      
      // For debugging: show all values are from balanceService
      source: 'balanceService.calculateAvailableBalance',
